...
def plot_emg_with_states(gesture_name, states_to_include, emg_signals_to_include=None, color_shading=True,
    ...
    file_path = f"Archive/Data_CleanUp_L/merged_file_fist_L_cleaned.csv"  # Adjust as needed
    ...
    df["ElapsedTime"] = (df["Timestamp"] - df["Timestamp"].iloc[0]).dt.total_seconds()
    ...
    emg_signals_to_include = emg_signals_to_include or list(range(1, 9))

    # Plot only the selected EMG signals
    for i in emg_signals_to_include:
        plt.plot(df["ElapsedTime"], df[f"EMG{i}"], label=f"EMG{i}")
            ...
            if current_state in states_to_include:
                if prev_state != current_state:
                    if start_time is not None:
                        plt.axvspan(start_time, row["ElapsedTime"], color=state_colors.get(prev_state, "gray"), alpha=0.3)
                    start_time = row["ElapsedTime"]
                prev_state = current_state
        if start_time is not None:
            plt.axvspan(start_time, df["ElapsedTime"].iloc[-1], color=state_colors.get(prev_state, "gray"), alpha=0.3)

    # Add vertical dashed lines at state change points
    if show_lines:
        for _, row in state_changes.iterrows():
            plt.axvline(x=row["ElapsedTime"], color="black", linestyle="--", alpha=0.6)
            plt.text(row["ElapsedTime"], min_emg_value - 10, row["State"],
                     rotation=90, verticalalignment='bottom', fontsize=16, color="black", fontweight='bold')

    # Formatting the plot
    plt.xlabel("Time (seconds)", fontsize=16, fontweight='bold')
    plt.ylabel("EMG Value", fontweight='bold', fontsize=16)
    plt.title(f"EMG Signals Over Time with State Annotations ({gesture_name.capitalize()} Gesture)",
